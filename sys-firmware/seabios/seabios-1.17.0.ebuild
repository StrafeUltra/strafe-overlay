...
src_prepare() {
	default
	# Ensure precompiled iasl outputs are never used
...
src_configure() {
	...
	# Provide a non-interactive config and lock it in
	cp "${FILESDIR}/seabios/config.seabios-256k" .config || die
	if use debug ; then
		sed -i -e 's/^CONFIG_DEBUG_LEVEL=.*/CONFIG_DEBUG_LEVEL=8/' .config || die
	fi
	_emake olddefconfig
}
...
src_compile() {
	...
	# Build main BIOS (iasl will be invoked automatically if needed)
